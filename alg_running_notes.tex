...
We don't go through the Gabow construction, as it's a bit tricky.
However, we'll talk a bit about scaling algorithms for SSSP in general.
...
get Disjkstra's algorithm down to $O(|E| + |V|\log|V|) = O(m + n\log n)$.
...
We claim that forall $i$, we have $e_i \leq d_i$.
