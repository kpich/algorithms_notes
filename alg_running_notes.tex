...
Well, we order the elements $(x_i, w_i)$ by decreasing order
...
$B'$ by growing an $A^*$ or something,
...
often get faster algorithms for specific matroids.
...
So if we have some operations that are much more expensive than others, but
the overall cost is less than the pessimistic estimate (wherein we upper-bound
...
\begin{enumerate}
	...
	operations
	will take $O(k)$; then, when an operation is performed, we'll want to
	give an operation a constant amount of dollars in order to do it (we'll
	...
	The amortized cost of an operation will be the actual cost plus
	...
	This is tricky---what we really care about is the total cost, rather
	...
	When we sum the total cost, we get the potential telescoping: the sum
...
We have a cheap operation, pop (push a single item from the stack),
...
\begin{itemize}
	...
	we can easily prove taht each element has one dollar associated with
	it, and so we'll be able to fund every operations.
